...
export class Socket
{
    ...
    public static init(server){
        ...
        this.IO  = require('socket.io')(server,{
            cors: {
                origin: ["https://app.sportyeah.com","https://admin.sportyeah.com","https://www.sportyeah.com","http://localhost:8100"],
                methods: ["GET", "POST"]
              }
        })
    ...
    public static async pushUser(socketID,userID){
        User.updateOne(userID,{connected:true}).then((user)=>{
    ...
    private static userDisconnet(id){
        ...
        if(user){
            User.updateOne(user.idUser,{connected:false,lastConection:new Date()})
            .then((user)=>{
